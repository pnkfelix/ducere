...
struct EarleyKey(usize, usize, State, State);
...
#[derive(Debug)]
pub(crate) struct EarleyTrees(Vec<Vec<HashMap<EarleyKey, HashMap<expr::Env, Vec<Tree>>>>>);

impl EarleyTrees {

}

type EarleyMap = HashMap<EarleyKey, HashMap<expr::Env, Vec<Tree>>>;
type IJ = (usize, usize);
...
impl EarleyConfig {
    ...
    pub fn new_with_binding(earley: Earley, x: expr::Var, v: expr::Val) -> Self {
        ...
        let env_trees: HashMap<_, _> = vec![(env, vec![Tree(vec![])])].into_iter().collect();
        let mut trees_0_0 = HashMap::new();
        trees_0_0.insert(EarleyKey(0, 0, q_0, q_0), env_trees);
    ...
    fn apply_rules(&mut self, t: Term) -> Changed {
        ...
        if let Changed::Changed = dbg!(self.apply_et_term(t)) { return Changed::Changed; }
    ...
    fn apply_et_term(&mut self, t: Term) -> Changed {
        ...
        for i in 0..j {
            ...
            for (&EarleyKey(_, _, q, r), ref env_trees) in tree_i_jsub1 {
    ...
    fn apply_et_pred(&mut self) -> Changed {
        ...
        for i in 0..=j {
            let mut to_add: Vec<(EarleyKey, expr::Env, Vec<Tree>)> = Vec::new();
            let tree_i_j = self.trees.get_map_mut((i, j));
            for (&EarleyKey(_, _, q, r), env_trees) in &*tree_i_j {
                for (ref env, trees) in env_trees.iter() {
                    for (e, &s) in self.earley.transducer().data(r).pred_transitions() {
                        if let expr::Val::Bool(true) = e.eval(env) {
                            to_add.push((EarleyKey(i, j, q, s), (*env).clone(), trees.clone()));
                        }
                    }
                }
            }
    ...
    fn apply_et_bind(&mut self) -> Changed {
        ...
        for i in 0..=j {
            let mut to_add: Vec<(EarleyKey, expr::Env, Tree)> = Vec::new();
            let tree_i_j = self.trees.get_map_mut((i, j));
            for (&EarleyKey(_, _, q, r), ref env_trees) in &*tree_i_j {
                for (ref env, trees) in env_trees.iter() {
                    for ((x, e), &s) in self.earley.transducer().data(r).bind_transitions() {
                        ...
                        for t in trees {
                            ...
                            to_add.push((EarleyKey(i, j, q, s), env.clone(), t));
                        }
                    }
                }
            }
            for (key, env, tree) in to_add {
                let trees = tree_i_j.entry(key).or_insert(HashMap::new());
                trees.entry(env.clone()).or_insert(vec![]);
                let cell = trees.get_mut(&env).unwrap();
                if !cell.contains(&tree) {
    ...
    fn apply_et_return(&mut self) -> Changed {
        ...
        for i in 0..=j {
            ...
            for k in 0..=j {
                ...
                for (&EarleyKey(_, _, s, t), env_trees_2) in tree_i_j {
                    for (ref env_2, trees_2) in env_trees_2.iter() {
                        for &(ref e_1, q) in transducer.data(t).calls() {
                            for (&EarleyKey(_, _, q_, r), env_trees_1) in tree_k_j {
                                for (ref env_1, trees_1) in env_trees_1.iter() {
                                    ...
                                    for ((opt_x, nt, opt_e), &u) in transducer.data(t).nonterm_transitions(non_terms) {
                                        ...
                                        for t_2 in trees_2 {
                                            for t_1 in trees_1 {
                                                ...
                                                let key = EarleyKey(i, j, s, u);
                                                to_add.push((key, env_2.clone(), t_2));
                                            }
                                        }
                                    }
                                }
